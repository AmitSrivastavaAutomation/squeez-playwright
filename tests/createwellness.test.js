...
test('Login and create new wellness entry from Excel', async ({ page }) => {
  ...
  await page.screenshot({ path: `before-fill-wellness-${currentIndex + 1}.png`, fullPage: true });

  await wellnessPage.fillForm(wellnessData);

  await page.screenshot({ path: `before-submit-wellness-${currentIndex + 1}.png`, fullPage: true });
  await wellnessPage.submit();
  await page.screenshot({ path: `after-submit-wellness-${currentIndex + 1}.png`, fullPage: true });
